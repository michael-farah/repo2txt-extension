...
import type { ParsedRepoInfo, CacheAdapter } from '@/lib/providers/types';
import type { ProviderType, FileNode, FetchOptions, FileContent } from '@/types';
import { logger } from '@/lib/utils/logger';
import { sendMessage, MessageTypes } from '@/lib/chrome/messages';
import type { GithubWebFetchRequest, GithubWebFetchResponse } from '@/lib/chrome/messages';
...
export class GitHubProvider extends BaseProvider {
  ...
  /** Cache adapter for repo data (injected by caller) */
  private cacheAdapter: CacheAdapter | null = null;

  /**
   * Set the cache adapter for reading/writing cached repo data
   */
  ...
  private async sessionFetch(url: string, signal?: AbortSignal): Promise<string> {
    let response: GithubWebFetchResponse | undefined;
    ...
    try {
      response = await sendMessage<GithubWebFetchRequest, GithubWebFetchResponse>({
    ...
    if (response.success) {
      logger.debug(
        'repo2txt',
        `sessionFetch: ${url} → ${response.status} (${response.html.length} bytes)`
      );
    }
  ...
  private parseRepoPage(html: string): {
    ...
    if (files.length === 0) {
      ...
      if (files.length === 0) {
        logger.warn(
          'repo2txt',
          'parseRepoPage: embeddedData parsing found 0 files, trying DOM fallback'
        );
      }
    }
    ...
    logger.debug(
      'repo2txt',
      `parseRepoPage: found ${files.length} files, defaultBranch=${defaultBranch}, currentBranch=${currentBranch ?? 'none'}`
    );
  ...
  async fetchTree(url: string, options?: FetchOptions): Promise<FileNode[]> {
    ...
    const cached = this.cacheAdapter?.getCachedRepo(cacheKey);
    ...
    try {
      ...
      if (this.sessionMode) {
        ...
        this.cacheAdapter?.setCachedRepo(cacheKey, tree);
        return tree;
      }
