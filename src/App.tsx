...
function App() {
  ...
  const { initialUrl, autoSubmitUrl, resetTabState } = useChromeTab(isLoading, () =>
  ...
  const resetAll = useCallback(() => {
    ...
    resetTabState();
  }, [resetProviderState, setShowExcludedFiles, resetTabState]);
  ...
  const {
    ...
    getDirectorySelectionState,
    getFullTree: () => useStore.getState().getTree({ fullyExpanded: true, filterExcluded: true }),
  ...
  // Calculate flat node counts for search
  ...
  const totalNodeCount = flatNodeCount;
        ...
        <div className="space-y-4">
          ...
          {(output || isGenerating) && (
            <section ref={outputRef}>
              <h2 className="text-sm font-semibold text-content mb-2">Output</h2>
              <OutputPanel
                output={output}
